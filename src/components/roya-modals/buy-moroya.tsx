...
import { Loader } from '..';
import { buyMoroyaLot } from '../../logic/actions';
import {
  StakingLotModals,
...
const BuyMoroya = (props: Props) => {
  ...
  const handleBuyMoroya = (values: any) => {
    const { lotAmount, nftDiscount } = values;

    if (walletConnected) {
      dispatch(buyMoroyaLot(userAddress, lotAmount, nftDiscount));
    }
  };
    ...
    lotAmount: Yup.string()
      ...
      .test(
        ...
        (val) =>
          ...
          BigInt(
            toRoya((parseInt(val as string) * LotsPrice.monarchLot).toString())
          )
      ),
  ...
  return (
    ...
    >
      {({ values, setFieldValue }) => {
        ...
        return (
          <Form>
            <Header>moROYA Staking Lot</Header>
            <LotSelectContainer>
              <LotSelect>
              ...
              <LotTextSecondary className="notranslate">
                {thousandSeparator(LotsPrice.monarchLot.toString())} ROYA
              </LotTextSecondary>
              <LotTextSecondary>=</LotTextSecondary>
              <LotTextSecondary className="notranslate">
                {thousandSeparator(
                  (LotsPrice.monarchLot * lotAmount).toString()
                )}{' '}
                ROYA
              </LotTextSecondary>
            </LotSelectContainer>
