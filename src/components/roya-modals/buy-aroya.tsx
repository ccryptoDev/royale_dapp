...
import { Loader } from '..';
import { buyAroyaLot } from '../../logic/actions';
import {
  getConfig,
  fromRoya,
...
const BuyAroya = (props: Props) => {
  ...
  const handleBuyAroya = (values: any) => {
    const { lotAmount, nftDiscount } = values;

    if (walletConnected) {
      dispatch(buyAroyaLot(userAddress, lotAmount, nftDiscount));
    }
  };
    ...
    lotAmount: Yup.string()
      ...
      .test(
        ...
        (val) =>
          ...
          BigInt(
            toRoya((parseInt(val as string) * LotsPrice.archonLot).toString())
          )
      ),
  ...
  return (
    ...
    >
      {({ values, setFieldValue }) => {
        ...
        return (
          <Form>
            <Header>aROYA Staking Lot</Header>
            ...
            <LotSelectContainer>
              ...
              <LotTextSecondary className="notranslate">
                {thousandSeparator(LotsPrice.archonLot.toString())} ROYA
              </LotTextSecondary>
              <LotTextSecondary>=</LotTextSecondary>
              <LotTextSecondary className="notranslate">
                {thousandSeparator(
                  (LotsPrice.archonLot * lotAmount).toString()
                )}{' '}
                ROYA
              </LotTextSecondary>
            </LotSelectContainer>
