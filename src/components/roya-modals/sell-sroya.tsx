...
import { sellSroyaLot } from '../../logic/actions';
import { Loader } from '..';
...
const SellSroya = (props: Props) => {
  const { walletConnected, userAddress } = useSelector(
    (state: any) => state.user
  );

  const { lotOperation, totalSroyaLot } = useSelector(
  ...
  const handleSellSRoya = (values: any) => {
    const { lotAmount, nftDiscount } = values;

    if (walletConnected) {
      dispatch(sellSroyaLot(userAddress, parseInt(lotAmount), nftDiscount));
    }
  };
  ...
  const renderRoyaText = (value: string): string => {
    if (!isNaN(parseInt(value))) {
      ...
      if (lotAmount >= 1) {
        return `${(lotAmount * LotsPrice.settlerLot).toLocaleString()} ROYA`;
      }
  ...
  const schema = Yup.object().shape({
    lotAmount: Yup.string()
      ...
      .test(
        ...
        (val) => parseInt(val) <= parseInt(totalSroyaLot)
      ),
  ...
  return (
    <Formik
      ...
      onSubmit={handleSellSRoya}
    >
      {({ setFieldValue, values }) => (
        <Form>
          <Header>UNSTAKE sROYA</Header>
          <LotsFormContainer>
            <StakingLotsField
              name="lotAmount"
            ...
            <MaxButton
              type="button"
              onClick={() => setFieldValue('lotAmount', totalSroyaLot)}
            >
              MAX
            </MaxButton>
          </LotsFormContainer>
          ...
          <LotAmountText>
            ...
            <span className="notranslate">
              {!!totalSroyaLot && `${totalSroyaLot} sROYA`}
            </span>
          </LotAmountText>
